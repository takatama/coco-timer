(() => {
  const recipe = {
    ...
    steps: [
      ...
      { timeSec: 210, actionType: "none", name: { ja: "完成", en: "Finish" }, waterAmountType: null },
    ],
  };
  ...
  const calcFlavor1 = () => {
    ...
    const factor = state.flavor === "sweet" ? 0.42 : state.flavor === "sour" ? 0.58 : 0.5;
    return Math.round(total * 0.4 * factor);
  };

  const calcFlavor2 = () => {
    const total = getTotalWater();
    const factor = state.flavor === "sweet" ? 0.58 : state.flavor === "sour" ? 0.42 : 0.5;
    return Math.round(total * 0.4 * factor);
  };
  ...
  const updateMainCard = () => {
    ...
    const stepEnd = nextStep ? nextStep.timeSec : computedSteps[computedSteps.length - 1].timeSec;
  ...
  const updateSummary = () => {
    ...
    const flavorLabelJa = state.flavor === "sweet" ? "甘味" : state.flavor === "sour" ? "酸味" : "バランス";
    const flavorLabelEn = state.flavor === "sweet" ? "Sweet" : state.flavor === "sour" ? "Sour" : "Balance";
    const flavorLabel = state.lang === "ja" ? flavorLabelJa : flavorLabelEn;
    elements.recipeParams.textContent = state.lang === "ja"
  ...
  const tick = () => {
    ...
    for (let i = 0; i < speed; i += 1) {
      ...
      if (nextStep && remainingToNext === 5 && state.lastAnnouncedStep !== currentIndex + 1) {
      ...
      if (!nextStep && finalTime - state.currentTime === 5 && !state.lastFinishAnnounced) {
  ...
  const applyLanguage = () => {
    ...
    elements.labelAnimation.textContent = state.lang === "ja" ? "アニメーション" : "Animation";
  ...
  const loadSettings = () => {
    ...
    try {
      ...
      if (typeof parsed.debugSpeed === "number") state.debugSpeed = parsed.debugSpeed;
    } catch {}
  };
  ...
  const init = () => {
    ...
    if (["sweet", "neutral", "sour"].includes(flavorParam)) state.flavor = flavorParam;
    ...
    if (window.location.protocol === "file:") {
      ...
      Array.from(elements.animChoices.querySelectorAll(".choice")).forEach((btn) => {
        btn.disabled = true;
      });
      applySettingsUI();
    }
